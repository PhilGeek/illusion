...
This contrast between the appearance of the Benham disk and the appearance of the pointillist painting suggests the following dilemma. On the standard conception, a visual illusion is an experience of the object \( o \) looking \( F \) where \( o \) is not \( F \). \( F \) is a way for things to be, and the quality designated by \( F \) is attributed in experience to the object seen. However, the putative examples of color illusion that we have so far considered are either cases:
	\begin{itemize}
	...
	\end{itemize}
In the case of the pointillist painting, the painting looks green, ``green'' qualifies the painting, and the painting is green. In the case of the Benham disk, the ring looks reddish, ``reddish'' qualifies not the ring, but the ring's look---a look the ring genuinely has without the disk itself being reddish. The challenge to the standard conception is to find a case that uncontroversially falls into neither category but, rather, fits the following pattern: 
	\begin{itemize}
		\item where \( F \) qualifies \( o \) and \( o \) is not \( F \).
	\end{itemize}
If no such case can be found, then any conception of experience, such as representationalism, that is committed to the standard conception of illusion is false. 

No such case can be found. Nothing answers to the standard conception of illusion. Though I cannot decisively establish this, a review of some likely candidates supports this skepticism.

\subsection{Conflicting Appearances}\label{sub:conflicting_appearances} % (fold)

\todo{Discuss two cases: (1) interspecies perceptual variation in color vision, (2) cases of perceptual constancy.}
